      ...
      {slides.map((slide, index) => {
        ...
        return (
          ...
          >
            ...
            {/* Content Caption Overlay (Compact Sleek Layout) */}
            <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-6 flex flex-col justify-end text-white z-20">
              <div className="flex flex-wrap items-center gap-2 mb-1.5">
                <span className="px-3 py-0.5 rounded-full bg-[#27272a]/90 backdrop-blur-md text-white text-[11px] font-semibold tracking-wide flex items-center gap-1.5 shadow-md border border-white/10">
                  <Sparkles className="h-3 w-3 text-[#a1a1aa]" /> {slide.category}
                </span>
                <span className="text-[11px] font-medium text-[#a1a1aa] backdrop-blur-md bg-[#18181b]/80 px-2.5 py-0.5 rounded-full border border-white/10">
                  Build 0{index + 1} / 0{slides.length}
                </span>
              </div>
              
              <h3 className="text-xl sm:text-2xl font-bold font-sans text-white tracking-tight leading-tight mb-1 drop-shadow-md">
                {slide.title}
              </h3>
              
              <p className="text-xs sm:text-sm text-[#a1a1aa] line-clamp-2 max-w-xl font-sans leading-relaxed drop-shadow-xs mb-3">
                {slide.description}
              </p>
              ...
              {slide.video_url && (
                <div>
                  <a
                    ...
                    className="inline-flex items-center gap-2 text-xs font-semibold px-4 py-2 rounded-full bg-white hover:bg-neutral-200 text-black shadow-lg transition-all hover:scale-[1.02] font-sans"
                  >
